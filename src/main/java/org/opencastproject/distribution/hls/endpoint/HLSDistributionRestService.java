 ...
 *
 */
package org.opencastproject.distribution.hls.endpoint;
...
public class HLSDistributionRestService extends AbstractJobProducerEndpoint {
  ...
  public Response retract(@FormParam("mediapackage") String mediaPackageXml, @FormParam("elementId") String elementId)
    ...
    try {
      ...
      logger.warn("Unable to retract mediapackage '{}' from hls channel: {}", new Object[] { mediaPackageXml, e });
      return Response.serverError().status(Status.INTERNAL_SERVER_ERROR).build();
    }
    return Response.ok(new JaxbJob(job)).build();
  }
